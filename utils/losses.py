...
def _neg_loss(preds, targets):
  ...
  loss = 0
  for pred in preds:
    pred = torch.clamp(torch.sigmoid(pred), min=1e-4, max=1 - 1e-4).cpu()
...
def _theta_loss(theta, gt_theta, mask):
  mask = mask[:, :, None].expand_as(gt_theta).float()
  loss = sum([torch.mean((1-torch.cos(t * mask - gt_theta * mask))) for t in theta])
  return loss
