...
class exkp(nn.Module):
  def __init__(self, n, nstack, dims, modules, cnv_dim=256, num_classes=1):
    ...
    self.theta= nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 1) for _ in range(nstack)])
  ...
  def forward(self, image):
    ...
    for ind in range(self.nstack):
      ...
      if self.training or ind == self.nstack - 1:
        outs.append([self.hmap[ind](cnv), self.regs[ind](cnv), self.w_h_[ind](cnv), self.theta[ind](cnv)])
...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    outputs = net(torch.randn(2, 3, 512, 384))
  ...
  print("theta : ",theta[0].shape, theta[1].shape)
